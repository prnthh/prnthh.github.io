    ...
    useEffect(() => {
        ...
        const setup = async () => {
            ...
            const { success, navMesh, tileCache } = threeToTileCache(meshes, {
                tileSize: 16,
                cs: 0.1,              // finer horizontal grid (default 0.2) → more detail at mesh seams
                ch: 0.1,              // finer vertical grid (default 0.2)
                walkableClimb: 4,     // allow stepping over small height gaps at mesh intersections (vx)
                maxSimplificationError: 0.8, // preserve edge accuracy at boundaries (default 1.3)
            });
