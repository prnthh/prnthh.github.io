...
import MapPicker from "../killbox/MapPicker";
...
const RealGame = ({ game }: { game: ReturnType<typeof useFPSRoom> | null }) => {
    ...
    return (
        <div className="p-4 text-white">
            <div className="mb-4">
                ...
                <pre className="text-xs">{JSON.stringify(localEntity, null, 2)}</pre>
            </div>
            <div>
                <h3 className="text-lg font-bold">Other Players ({Object.keys(entities).length}):</h3>
                <pre className="text-xs">{JSON.stringify(entities, null, 2)}</pre>
            </div>
        </div>
    );
}