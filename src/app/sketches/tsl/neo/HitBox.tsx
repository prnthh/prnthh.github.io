...
const HitBox = ({ debug, children, position, ref }: { debug?: boolean; children?: React.ReactNode, position?: [number, number, number], ref?: React.Ref<RapierRigidBody> }) => {
    ...
    return (
        <RigidBody position={position} ref={ref} type={hitCount > 0 ? "dynamic" : "fixed"} onIntersectionEnter={(e) => {
            ...
            {children ?? <mesh castShadow>
                ...
                {debug && <Html position={[0, 0, 0]} center className="flex items-center justify-center bg-white w-[32px] h-[32px] rounded-full">{hitCount}</Html>}
            </mesh>}
        </RigidBody>
    );
}
