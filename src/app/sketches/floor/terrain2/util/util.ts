import { RepeatWrapping, NearestFilter, LinearMipmapNearestFilter, SRGBColorSpace } from "three";
import glsl from "glslify";

export function cartesian<T>(args: T[][]): T[][] {
	const r: T[][] = [], max = args.length-1;
...
export function getDimensions(texture: { source: { data: { width: number; height: number } } }) {
	return {
...
const map = <T, U>(f: (item: T) => U) => (arr: T[]): U[] => arr.map(f);
const filter = <T>(f: (item: T) => boolean) => (arr: T[]): T[] => arr.filter(f);
const pipe = (...fns: Array<(arg: any) => any>) => (x: any) => fns.reduce((v, f) => f(v), x);

export const pluck = <T, K extends keyof T>(key: K) => map((v: T) => v[key]);
export const defined = filter(v=>typeof v != undefined);
export const standard = (defaultValue: any) => map((v: any) => v || defaultValue);
export const repeatTexture = (t: any) => {
  t.wrapS = t.wrapT = RepeatWrapping;
  t.needsUpdate = true;
}
export const srgbTexture = (t: any) => {
	t.colorSpace = SRGBColorSpace;
	// t.encoding = LinearEncoding;
	t.needsUpdate = true;
  }
export const pixelateTexture = (t: any) => {
...
export const option = <T extends Record<string, any>, K extends keyof T>(arr: T[], key: K, defaultValue?: any) => pipe(
  pluck<T, K>(key),
