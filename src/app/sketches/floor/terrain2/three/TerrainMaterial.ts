...
export type TerrainMaterialOptions = THREE.MeshStandardMaterialParameters & {
  surfaces: Surface[];
...
export default class TerrainMaterial extends CustomShaderMaterial {
  ...
  constructor(props: TerrainMaterialOptions) {
    ...
    if (props.displacementMap && applyDefaultEncoding)
      props.displacementMap.colorSpace = THREE.LinearSRGBColorSpace;
    if (props.normalMap && applyDefaultEncoding)
      props.normalMap.colorSpace = THREE.LinearSRGBColorSpace;
  ...
  updateNestedUniform(name: string, value: any) {
  ...
  parseSurfaceData = (surface: Surface, i: number) => {
  ...
  initializeFarMaps(props: TerrainMaterialOptions) {
    if (this.farMaterial)
    ...
    const { width, height } = { width: maxSize * scale, height: maxSize * scale };
    this.farTargetDiffuse = new THREE.WebGLRenderTarget(width, height, { depthBuffer: false, format: THREE.RGBAFormat, generateMipmaps: true, minFilter: THREE.LinearMipmapLinearFilter, magFilter: THREE.LinearFilter, colorSpace: THREE.LinearSRGBColorSpace });
    this.farTargetNormal = new THREE.WebGLRenderTarget(width, height, { depthBuffer: false, format: THREE.RGBAFormat, generateMipmaps: true, minFilter: THREE.LinearMipmapLinearFilter, magFilter: THREE.LinearFilter, colorSpace: THREE.LinearSRGBColorSpace });
  }

  generateFarMaps(renderer: THREE.WebGLRenderer) {
  ...
  getMaxTextureSize(context?: WebGLRenderingContext) {
    const ctx = context ?? this.context;
    return ctx.getParameter(ctx.MAX_TEXTURE_SIZE);
  }

  renderTexture(renderer: THREE.WebGLRenderer, target: THREE.WebGLRenderTarget, scene: THREE.Scene, camera: THREE.Camera) {
